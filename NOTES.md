...
Every Git dependency is pinned to a full commit SHA in `lake-manifest.json`. In `lakefile.toml` Mathlib is required by its tag `v4.35.0-rc1`, and jordan_pick and leancert by commit. Mathlib is required last, so that its versions of shared transitive dependencies win. The three directories under `vendor/` are built as libraries of this package (`srcDir` in `lakefile.toml`), not as path dependencies: Comparator lets a build write only to the project's own `.lake`, and a path dependency builds into its own directory. `vendor/gerver-sofa` still carries the `lakefile.toml`, manifest and `lean-toolchain` of the upstream release, which Lake ignores.
...
| Tau Ceti | `vendor/tauceti`, ten files copied unchanged from commit `c52a81811e4626d2e9769fe9b0c701168c211332` | Apache-2.0, © 2026 The Tau Ceti contributors | `isJordanCurve_range_union_range_of_inter_eq_pair` (a Jordan curve from two concatenated arcs meeting in their endpoints), `filledHull_subset_closedConvexHull`, and lemmas on bounded variation |
| lean-pool | `vendor/lean-pool`, three files copied unchanged from commit `bb74ee07fc23bc81358d75a9c40303e5e27fced8` | Apache-2.0, © 2026 Jonathan Ho | `brunn_minkowski_euclideanSpace`, Jonathan Ho's Brunn–Minkowski inequality and its Prékopa–Leindler prerequisites |
...
The ten Tau Ceti files are vendored because Palomar requires a project that uses Tau Ceti as a Git package to build against exactly the Mathlib revision in Tau Ceti's own manifest. At the commit used, that is a Mathlib for Lean `v4.34.0-rc2`, and this project is on Mathlib `v4.35.0-rc1`. The files are the four modules imported here and the six Tau Ceti modules they import.

The three lean-pool files are vendored because that package is named `lean-pool`. Lake writes the name into `lake-manifest.json` as `«lean-pool»`, and Palomar's verifier rejects package names with those characters.
